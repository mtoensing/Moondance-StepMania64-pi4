...
for i = 1,#tNames do
    Carts[#Carts+1] = Def.ActorFrame {
        OnCommand=function(self) self:x(480):linear(80):x(-4800) end,
        Def.Sprite {
...
Carts[#Carts+1] = Def.ActorFrame {
	...
	Def.Sprite {
		Texture="fox.png",
		InitCommand=function(self) self:SetTextureFiltering(false):xy(-180+4+(96*(#Carts-1)),-2):zoom(.1):sleep(6.5+(#Carts*1.3)):diffusealpha(0) end 		
	},
	Def.Sprite {
...
return Def.ActorFrame {
	Def.ActorFrame {
		InitCommand=function(self)
			self:zoom((SCREEN_HEIGHT/720)*3):xy(SCREEN_CENTER_X,SCREEN_CENTER_Y+((SCREEN_HEIGHT/720)*300)):sleep(9999)
		end,
			...
			for pn = 1,2 do
				...
				if GAMESTATE:IsPlayerEnabled("PlayerNumber_P"..pn) then			
					for _,col in ipairs(SCREENMAN:GetTopScreen():GetChild("PlayerP"..pn):GetChild("NoteField"):get_column_actors()) do
						col:sleep(9+(#Carts*1.3)):linear(.5):diffusealpha(.25)
					end
				end
			end
		end,
		...
		Def.Sprite {
			...
			OnCommand=function(self) self:diffusealpha(0):linear(0.5):diffusealpha(1):sleep(9.5+(#Carts*1.3)):queuecommand("Scroll") end,
		...
		Def.Sprite {
			...
			InitCommand=function(self) self:animate(false):setstate(0):SetTextureFiltering(false):diffusealpha(0):sleep(6.5+(#Carts*1.3)):diffusealpha(1):zoom(0.15):decelerate(0.5):y(-50):accelerate(0.5):y(0):sleep(2.5):queuecommand("Walk") end,
